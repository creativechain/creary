...
(function () {
                ...
                methods: {
                    ...
                    getFeaturedImage: function getFeaturedImage(post) {
                        ...
                        if (
                            ...
                            (this.account &&
                                this.account.user.metadata.adult_content ===
                                    "warn" &&
                                post.adult_content)
                        ) {
                            featuredImage = {
                    ...
                    getPayout: function getPayout(post) {
                        ...
                        if (this.hasPaid(post)) {
                            ...
                            amount = amount.add(
                                Asset.parseString(post.curator_payout_value)
                            );
                    ...
                    getPendingPayouts: function getPendingPayouts(post, asset) {
                        ...
                        let PRICE_PER_CREA = Asset.parse({
                            amount:
                                Asset.parseString(
                                    this.state.feed_price.base
                                ).toFloat() /
                                Asset.parseString(
                                    this.state.feed_price.quote
                                ).toFloat(),
                            nai: "cbd",
                        });
                        ...
                        let PERCENT_CREA_DOLLARS =
                            post.percent_crea_dollars / 20000;
                        let PENDING_PAYOUT_CBD = Asset.parse({
                            amount:
                                PENDING_PAYOUT.toFloat() * PERCENT_CREA_DOLLARS,
                            nai: "cbd",
                        });
                        let PENDING_PAYOUT_CGY = Asset.parse({
                            amount: NaNOr(
                                (PENDING_PAYOUT.toFloat() -
                                    PENDING_PAYOUT_CBD.toFloat()) /
                        ...
                        let PENDING_PAYOUT_PRINTED_CBD = Asset.parse({
                            amount: NaNOr(
                                PENDING_PAYOUT_CBD.toFloat() *
                                    (CBD_PRINT_RATE / CBD_PRINT_RATE_MAX),
                                0
                            ),
                            nai: "cbd",
                        });
                        let PENDING_PAYOUT_PRINTED_CREA = Asset.parse({
                            amount: NaNOr(
                                (PENDING_PAYOUT_CBD.toFloat() -
                                    PENDING_PAYOUT_PRINTED_CBD.toFloat()) /
                                    PRICE_PER_CREA.toFloat(),
                                0
                            ),
                            nai: "crea",
                        ...
                        switch (asset) {
                            case "cgy":
                                return PENDING_PAYOUT_CGY.toFriendlyString(
                                    null,
                                    false
                                );
                            case "cbd":
                                return PENDING_PAYOUT_PRINTED_CBD.toFriendlyString(
                                    null,
                                    false
                                );
                            case "crea":
                                return PENDING_PAYOUT_PRINTED_CREA.toFriendlyString(
                                    null,
                                    false
                                );
                            default:
                                return (
                                    "(" +
                                    PENDING_PAYOUT_PRINTED_CBD.toFriendlyString(
                                        null,
                                        false
                                    ) +
                                    ", " +
                                    PENDING_PAYOUT_PRINTED_CREA.toFriendlyString(
                                        null,
                                        false
                                    ) +
                                    ", " +
                                    PENDING_PAYOUT_CGY.toFriendlyString(
                                        null,
                                        false
                                    ) +
                                    ")"
    ...
    creaEvents.on(
        "crea.posts",
        function (urlFilter, filter, state, hasContent, needCleanContent) {
            let authors = [];
            let category = getPathPart(urlFilter);
            let discuss = getPathPart(urlFilter, 1) || "";
            console.log(
                    ...
                    if (!catchError(err)) {
                        ...
                        if (homePosts) {
                            ...
                            for (let _c in state.content) {
                                ...
                                if (
                                    !homePosts.state.discussion_idx["feed"][
                                        getPathPart()
                                    ].includes(_c)
                                ) {
                                    homePosts.state.discussion_idx["feed"][
                ...
                if (homePosts) {
                    ...
                    if (homePosts.urlFilter === urlFilter) {
                        for (let a in state.accounts) {
                            homePosts.state.accounts[a] = parseAccount(
                                state.accounts[a]
                            );
                        }
                    }
        ...
        let onHomeInit = function () {
                    ...
                    getAccounts(accountNames, function (err, newAccounts) {
                        if (!catchError(err)) {
                            ...
                            newAccounts.forEach(function (a) {
                                homePosts.state.accounts[a.name] = parseAccount(
                                    a
                                );
                            });
            ...
            apiContents.forEach(function (d) {
                ...
                if (!homePosts.state.content[permlink]) {
                    ...
                    homePosts.state.content[permlink].reblogged_by =
                        d.reblogged_by;
                    onContentFetched();
                }
            });
    ...
    creaEvents.on(
        "crea.content.add",
        function (apiContents, accountNames, discussion_idx, cleanContent) {
            addApiContent(
                apiContents,
                accountNames,
                discussion_idx,
                ...
                if (lastPage) {
                    switch (category) {
                        ...
                        default:
                            apiCall =
                                crea.api[
                                    "getDiscussionsBy" + category.capitalize()
                                ];
                            break;
                    }
                                ...
                                if (err) {
                                                ...
                                                if (!catchError(err)) {
                                                    ...
                                                    topDiscussions.forEach(
                                                        function (d) {
                                                            ...
                                                            homePosts.state.discussion_idx[
                                                                discuss
                                                            ][category].push(
                                                                permlink
                                                            );
                                    ...
                                    if (discussions.length) {
                                        ...
                                        for (
                                            ...
                                            if (category === "now") {
                                                let postCreatedDate = moment(
                                                    d.created,
                                                    "YYYY-MM-DDTHH:mm:ss"
                                                );
                                                let postPayoutDate = moment(
                                                    d.last_payout,
                                                    "YYYY-MM-DDTHH:mm:ss"
                                                );
                                                if (
                                                    postCreatedDate.isAfter(
                                                        postPayoutDate
                                                    )
                                                ) {
                                                    //Post is active
                                        ...
                                        let onReblogs = function (dr) {
                                            ...
                                            topDiscussions[d.index] = parsePost(
                                                d,
                                                dr.reblogged_by
                                            );
                                        };
        ...
        let onFinish = function onFinish(state) {
            ...
            if (count >= data.length) {
                ...
                searchState.discussion.sort(function (c1, c2) {
                    return (
                        state.content[c2].active_votes.length -
                        state.content[c1].active_votes.length
                    );
                });
            ...
            let getState = function getState(r) {
                ...
                crea.api.getState(url, function (err, result) {
                    if (err) {
                        ...
                        searchState.accounts[r.author] =
                            result.accounts[r.author];
                        searchState.content[permalink] =
                            result.content[permalink];
                        onFinish(result);
                    }
                });
            };
