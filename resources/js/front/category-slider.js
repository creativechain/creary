...
const categorySlider = function() {
  ...
  const render = function() {
    ...
    const edgeRight = categoryList.scrollLeft + categoryList.clientWidth >= categoryList.scrollWidth;
  ...
  const categoryClicked = function($event) {
    ...
    const categoryCurrentPosition = $event.currentTarget.offsetLeft + $event.currentTarget.clientWidth;
    const edgeLimitRight = categoryList.clientWidth + categoryList.scrollLeft - nextArrow.clientWidth;
    const offsetToLeft = $event.currentTarget.offsetLeft - backArrow.clientWidth;
    const offsetToRight = $event.currentTarget.offsetLeft - nextArrow.clientWidth ;
    ...
    if (edgeLimitRight < categoryCurrentPosition) {
      categoryList.scrollLeft = categoryList.scrollLeft + categoryCurrentPosition - edgeLimitRight;
    } 
  }

  const forceActivePosition = function() {
    const currentCatWidth = currentCategory.offsetLeft + currentCategory.clientWidth;
    const restCurrentCatWidth = categoryList.clientWidth - nextArrow.clientWidth;
    const currentCatNearToEnd = currentCategory.offsetLeft - nextArrow.clientWidth;
    
    if (currentCategory && (currentCatWidth < restCurrentCatWidth || currentCatNearToEnd)) {
