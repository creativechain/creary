...
Route::group(['prefix' => 'accounts', 'middleware' => ['cors']], function () {
    ...
    Route::middleware(['throttle:150,1'])->get('/search', 'Api\AccountsController@search')->name('accounts.search');
});

Route::group(['prefix' => 'tags', 'middleware' => ['cors']], function () {

    Route::get('/', 'Api\TagsController@index')->name('tags.index');
    Route::middleware(['throttle:150,1'])->get('/search', 'Api\TagsController@search')->name('tags.search');
});

Route::group(['prefix' => 'comments', 'middleware' => ['cors']], function () {

    //Route::get('/', 'Api\TagsController@index')->name('tags.index');
    Route::middleware(['throttle:150,1'])->get('/feed', 'Api\CommentsController@feed')->name('comments.feed');
    Route::middleware(['throttle:150,1'])->get('/searchByReward', 'Api\CommentsController@searchByReward')->name('comments.searchByReward');
